...
class Crex(private val domains: DomainResolver, private val artwork: ChannelArtwork) : MainAPI() {
    override var mainUrl = DomainResolver.GATEWAY
    override var name = "Crex • TurkSpor"
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        var found = false
        for ((index, player) in channel.players.take(5).withIndex()) {
            try {
                var referer = domains.currentUrl
                val stream = if (URI(player).path.endsWith(".m3u8", true)) player else {
                    val page = app.get(player, referer = referer, headers = mapOf("User-Agent" to DomainResolver.UA), timeout = 10)
                    if (page.code != 200) continue
                    referer = URI(page.url).let { "${it.scheme}://${it.authority}/" }
                    SportsParser.mappedStream(page.text, page.url) ?: run {
                        val endpoint = SportsParser.domainEndpoint(page.text) ?: return@run null
                        val response = app.get(endpoint, referer = referer, headers = mapOf("User-Agent" to DomainResolver.UA), timeout = 8)
                        if (response.code != 200) return@run null
                        val base = SportsParser.baseFromJson(response.text) ?: return@run null
                        SportsParser.royalStream(base, page.url)
                    }
                } ?: continue
                if (!seen.add(stream)) continue
                val requestHeaders = mapOf("User-Agent" to DomainResolver.UA, "Origin" to referer.trimEnd('/'))
                val playlist = app.get(stream, referer = referer, headers = requestHeaders, timeout = 10)
                if (playlist.code != 200 || !playlist.text.trimStart().startsWith("#EXTM3U")) continue
                links.addAll(turkspor.common.HlsQuality.links(name,"${channel.title} • Kaynak ${index + 1}",playlist.url,playlist.text,referer,requestHeaders))
                found = true
            } catch (e: CancellationException) { throw e } catch (_: Exception) { }
        }
        if (!found) throw ErrorLoadingException("Bu kanalın kaynakları şu anda yanıt vermiyor; maç saatinde veya WARP ile tekrar deneyin.")
