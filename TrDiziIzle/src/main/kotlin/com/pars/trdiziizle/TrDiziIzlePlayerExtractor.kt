...
/**
 * trdiziizle.tv'nin kendi domaininde barinan /player/oynat/{hash} sayfasini cozer.
 *
 * Ddizi.im ile ayni yol yapisini kullaniyor (/player/oynat/{hash}) ve ayni WP
 * tema ailesinden goruluyor; basit Referer tabanli koruma bekleniyor (FilmMakinesi/
 * 720izle'deki gibi AES sifreleme YOK gibi gorunuyor). Ancak bu sayfanin ic HTML/JS
 * yapisini Referer korumasi nedeniyle statik analizle goremedik, bu yuzden Ddizi'de
 * kullanilan ayni savunmaci, cok-desenli yaklasimi uyguluyoruz:
 *   1) Sayfa icinde baska bir <iframe> varsa (gercek video host'una yonlendirme)
 *   2) Duz metin m3u8/mp4/mpd linki varsa
 *   3) JWPlayer/Clappr tarzi "file:"/"source:" JS degiskeni varsa
 * Hicbiri bulunamazsa detayli log birakiyor, boylece Logcat'ten kesin
 * teshis konulabilir. CIHAZDA TEST EDILMEDI - ilk calistirmada Logcat'teki
 * TRDIZIIZLE_PLAYER etiketli PLAYER_HTML/PLAYER_NO_MATCH_FOUND satirlarina bakilmali.
 */
class TrDiziIzlePlayerExtractor : ExtractorApi() {
    override val name = "TrDiziIzle Player"
    override val mainUrl = "https://www.trdiziizle.tv"
    ...
    companion object {
        private const val TAG = "TRDIZIIZLE_PLAYER"
