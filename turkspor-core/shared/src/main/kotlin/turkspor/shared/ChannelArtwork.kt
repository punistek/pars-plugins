...
class ChannelArtwork(context: Context, key: String) {
    private val directory = File(context.cacheDir, "turkspor_${key}_cards_v1").apply { mkdirs() }
    private val semaphore = Semaphore(4)
    private fun file(channel: Channel) = File(directory, "${channel.id}.png")
    fun poster(channel: Channel): String = Uri.fromFile(file(channel)).toString()

    suspend fun prepare(channels: List<Channel>) = withContext(Dispatchers.IO) {
        coroutineScope {
            channels.distinctBy { it.id }.map { channel -> async { semaphore.withPermit { render(channel) } } }.awaitAll()
        }
    }
    private fun render(channel: Channel) {
        val output = file(channel)
        if (output.isFile && System.currentTimeMillis() - output.lastModified() < 24 * 60 * 60 * 1000) return
        val brand = Branding.forChannel(channel)
