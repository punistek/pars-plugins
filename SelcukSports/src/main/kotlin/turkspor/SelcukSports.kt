...
class SelcukSports(private val domains: DomainResolver, private val artwork: ChannelArtwork) : MainAPI() {
    override var mainUrl = DomainResolver.GATEWAY
    override var name = "SelçukSports • TurkSpor"
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        val playlist = app.get(stream, referer = playerOrigin, headers = mapOf("User-Agent" to DomainResolver.UA), timeout = 15)
        ...
        turkspor.common.HlsQuality.links(name,ChannelBranding.forChannel(channel).title,playlist.url,playlist.text,playerOrigin,mapOf("User-Agent" to DomainResolver.UA, "Origin" to playerOrigin.trimEnd('/'))).forEach(callback)
        return true
    }
}
