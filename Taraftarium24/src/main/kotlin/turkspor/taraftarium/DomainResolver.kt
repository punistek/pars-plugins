...
class DomainResolver(private val preferences: SharedPreferences) {
    companion object {
        const val GATEWAY = "https://taraftarium24.ch/"
        const val BOOTSTRAP = "https://taraftarium1081.xyz/"
    ...
    suspend fun resolve(force: Boolean = false): SiteSnapshot = mutex.withLock {
        ...
        try {
            ...
            if (response.code == 200) {
                ...
                if (root.optInt("schemaVersion") == 1) {
                    val urls = root.optJSONObject("sources")?.optJSONObject("taraftarium24")?.optJSONArray("candidates")
        ...
        candidates.add("https://inattv1322.xyz/")
        ...
        throw ErrorLoadingException("Taraftarium24 kaynağına ulaşılamadı. Eklenti ayarlarından Domaini yenile'yi deneyin. ${errors.lastOrNull().orEmpty()}")
    }
    ...
    suspend fun setManual(value: String): SiteSnapshot {
        ...
        val url = SportsParser.siteUrl(value) ?: throw ErrorLoadingException("Geçerli bir HTTPS Taraftarium24 adresi girin.")
